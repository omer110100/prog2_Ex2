...
errors_e load_responders(responder_s **responders, int *p_num_responders)
{
  ...
  {
    if((*p_num_responders) > MAX_NUM_RESPONDERS)
    {
...
errors_e assign_responder_to_incident(incident_type_e type, responder_s **responders, int num_responders, int *ind_chosen_responder)
{
  ...
  for (int i = 0; i < num_responders; i++)
  {
    if(relation[type][responders[i]->role] == 1)
    {
      ...
      if(num_handled_incidents > responders[i]->num_handled_incidents){
        index = i;
        num_handled_incidents = responders[i]->num_handled_incidents;
      }
    }    
...
errors_e handle_specific_incident(incident_node **p_head, responder_s **responders, int num_responders, int id)
{
  ...
  error_code = assign_responder_to_incident(
      incident->type, responders, num_responders, &chosen_responder_ind);
  ...
  printf("responder for incident %d is %s\n", incident->id,
         responders[chosen_responder_ind]->name);
...
errors_e handle_next_incident(incident_node **p_head, responder_s **responders, int num_responders)
{
  ...
  printf("responder for incident %d of type %d is %s of role %s\n",
         incident->id, incident->type, responder->name,
         role_types[responder->role]);
...
errors_e report_responder_by_name(responder_s **responders, int num_responders, char *name, responder_statistics_s *stat)
{
  ...
  if(responderIndex == -1)
  {
    return no_responder_found_by_name;
  }
...
printf("adding incident 123 security_alert priority 10\n");
error_code = add_incident( & head, 123, security_alert, 10, "robbery");
if (error_code != no_error) {
  printf("error code is %d\n", error_code);
}

printf("adding incident 101 zombie_attack priority 7\n");
error_code = add_incident( & head, 101, zombie_attack, 7, "herd of zombies from Colunia university");
if (error_code != no_error) {
  printf("error code is %d\n", error_code);
}

printf("adding incident 303 signaling_fault priority 5\n");
error_code = add_incident( & head, 303, signaling_fault, 5, "delays in signaling");
if (error_code != no_error) {
  printf("error code is %d\n", error_code);
}

printf("adding incident 444 obstacle_on_tracks priority 10\n");
error_code = add_incident( & head, 444, obstacle_on_tracks, 10, "car on tracks");
if (error_code != no_error) {
  printf("error code is %d\n", error_code);
}
...
printf("handling specific incident 101\n");
error_code = handle_specific_incident( & head, responders, num_responders, 101);
if (error_code != no_error) {
  printf("error code is %d\n", error_code);
}
...
printf("adding incident 777 ticketing disorders priority 2\n");
error_code = add_incident( & head, 777, ticketing_disorders, 2, "broken ticketing machine");
...
printf("handling specific incident 777\n");
error_code = handle_specific_incident( & head, responders, num_responders, 777);
if (error_code != no_error) {
  printf("error code is %d\n", error_code);
}
